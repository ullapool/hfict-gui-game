...
int CollisionDetection::checkBoundary(Obstacle *balloon, GameArea *area)
{
  ...
  if (balloon->center().y() <= 0 ||
      balloon->center().y() >= area->height()) return CollisionDetection::boundaryHorizontal;
  if (balloon->center().x() <= 0 ||
      balloon->center().x() >= area->width()) return CollisionDetection::boundaryVertical;
